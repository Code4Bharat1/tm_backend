...
export const punchOutController = async (req, res) => {
    try {
        ...
        if (!attendance.punchIn) {
            ...
            return res.status(200).json({ message: "Punch-in missing. Status set to Pending.", attendance });
        }
        ...
        res.status(500).json({ message: "Server error during punch-out", error });
    }
};
...
export const punchInController = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Server error during punch-in", error });
    }
};
